...
import com.rjs.myshows.common.util.saveImage
...
import com.rjs.myshows.services.domain.mdb.MdbGenre
import com.rjs.myshows.services.domain.mdb.MdbShow
import com.rjs.myshows.services.domain.mdb.MdbShowDetail
import com.rjs.myshows.services.domain.mdb.MdbShowListing
import com.rjs.myshows.services.service.ShowService
import com.rjs.myshows.services.service.ShowTypeService
import com.rjs.myshows.services.service.mdb.MdbService
...
class TmdbService(
    ...
    override fun addShow(showTypeName: String, mdbId: String): ShowEntity? {
        try {
            ...
            if (tmdbShow.id > -1) {
                var newShow = ShowEntity()
                newShow.mdbId = tmdbShow.id.toString()
                newShow.imdbId = tmdbShow.imdbId
                newShow.title = tmdbShow.title
                newShow.tagline = tmdbShow.tagline
                newShow.description = tmdbShow.overview
                newShow.runtime = tmdbShow.runtime
                newShow.showType = showTypeName
                newShow.genres.addAll(convertGenres(tmdbShow.genres, showTypeName))
    ...
    private fun downloadPosterImage(show: ShowEntity, tmdbPosterPath: String) {
        if (show.id == null) {
    ...
    private fun convertGenres(mdbGenres: Collection<MdbGenre>? = listOf(), showTypeName: String): MutableSet<String> {
        var showType: ShowTypeEntity? = showTypeService.findByName(showTypeName)
